...
const INPUTS = [
  ...
  { id: 'w', label: 'W', type: 'float', default: '(0)' },
];
const OUT_OUTPUT = { id: 'xyzw', label: 'XYZW', type: 'vec4' };

export function Vec4Node( { id, data } ) {
  ...
  const wDisplay = useNodeDisplayValueForInput( id, 'w' );

  const formatDisplay = ( inputId, val, connected ) => {
    if ( ! connected ) return typeof val === 'number' ? val : 0;
    const v = inputId === 'x' ? xDisplay : inputId === 'y' ? yDisplay : inputId === 'z' ? zDisplay : wDisplay;
  ...
  return (
    <div className={`tsl-node tsl-node--category-${category} tsl-node--compact`}>
      <div className="tsl-node__header">
        <div className="tsl-node__title">VEC4</div>
      </div>
